...
The stock kernel must allow loading modules (`CONFIG_MODULES`). The
`rdinit=/ethereal-init` trampoline loads the matching `ethereal.ko` from the
ramdisk and then executes the stock `/init`.
