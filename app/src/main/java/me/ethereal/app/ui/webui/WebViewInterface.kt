package me.ethereal.app.ui.webui
...
import me.ethereal.app.util.tryGetRootShell
...
class WebViewInterface(val context: Context, private val webView: WebView) {
    private val rootExecutor: ExecutorService = Executors.newSingleThreadExecutor { task ->
        Thread(task, "ethereal-webui-root").apply { isDaemon = true }
    }

    private fun submitRoot(block: () -> Unit) {
        runCatching { rootExecutor.execute(block) }
    }
    ...
    fun exec(cmd: String): String {
        ...
        return runCatching { ShellUtils.fastCmd(shell, cmd) }.getOrDefault("")
    }
    ...
    fun exec(
        ...
        submitRoot {
            ...
            val shell = tryGetRootShell() ?: return@submitRoot
    ...
    fun spawn(command: String, args: String, options: String?, callbackFunc: String) {
        submitRoot {
            ...
            val result = shell.newJob().add(finalCommand.toString()).to(stdout, stderr).exec()
            val emitExitCode =
