#!/system/bin/sh
############################################
# Ethereal module installer script
...
install_module() {
  ...
  if [ -n "$ETHEREAL_MODULE_STAGING" ]; then
    case "$ETHEREAL_MODULE_STAGING" in
      "$NVBASE/modules_update/".ethereal-stage-*) ;;
      *) abort "! Invalid Ethereal module staging path" ;;
    esac
    [ -d "$ETHEREAL_MODULE_STAGING" ] || abort "! Module staging directory is missing"
    [ ! -L "$ETHEREAL_MODULE_STAGING" ] || abort "! Module staging directory must not be a symlink"
    cp -af "$ETHEREAL_MODULE_STAGING/module.prop" "$TMPDIR/module.prop" || \
      abort "! Unable to read staged module.prop"
    TRANSACTION=true
  else
  ...
  case "$MODID" in
    *[!a-zA-Z0-9._-]*) abort "! Invalid module id: $MODID" ;;
  esac
  [ "${#MODID}" -le 128 ] || abort "! Module id is too long"
  if $TRANSACTION; then
    [ "$MODID" = "$ETHEREAL_MODULE" ] || abort "! Staged module id does not match"
    MODPATH=$ETHEREAL_MODULE_STAGING
  else
  ...
  if { $TRANSACTION && [ -f "$MODPATH/install.sh" ]; } || { ! $TRANSACTION && is_legacy_script; }; then
    if $TRANSACTION; then
      cp -af "$MODPATH/module.prop" "$TMPDIR/module.prop"
      [ -f "$MODPATH/install.sh" ] && cp -af "$MODPATH/install.sh" "$TMPDIR/install.sh"
      [ -f "$MODPATH/uninstall.sh" ] && cp -af "$MODPATH/uninstall.sh" "$TMPDIR/uninstall.sh"
      [ -d "$MODPATH/common" ] && cp -af "$MODPATH/common/." "$TMPDIR/"
      # Legacy on_install owns extraction. The Rust pre-extraction was only
      # used to validate the complete archive under strict limits.
      for ENTRY in "$MODPATH"/* "$MODPATH"/.[!.]* "$MODPATH"/..?*; do
    ...
    print_title "Powered by Ethereal"
    ...
    elif $TRANSACTION; then
      # Rust pre-extracts with strict limits. Preserve SKIPUNZIP semantics by
      # discarding that extraction before the module's customize.sh runs.
      for ENTRY in "$MODPATH"/* "$MODPATH"/.[!.]* "$MODPATH"/..?*; do
        [ -e "$ENTRY" ] || [ -L "$ENTRY" ] || continue
        case "${ENTRY##*/}" in
  ...
  $MODPATH/install.sh $MODPATH/common
  rmdir "$MODPATH" 2>/dev/null
