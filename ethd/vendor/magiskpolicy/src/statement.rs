...
impl SePolicy {
    ...
    fn load_rules_from_reader<T: BufRead>(&mut self, reader: &mut T) {
        for line in reader.lines() {
            match line {
                Ok(line) => self.parse_statement(&line),
                Err(err) => {
                    warn!("Failed to read policy statement: {err}");
                    break;
                }
            }
        }
...
pub fn format_statement_help(f: &mut dyn Write) -> std::fmt::Result {
    write!(
        f,
        r#"** Policy statements:
...
mod tests {
    ...
    fn parse(statement: &str) -> ParseResult<'_, PolicyStatement<'_>> {
        let mut tokens = tokenize_statement(statement).into_iter().peekable();
    ...
    fn test_parse_syntax_errors_and_corner_cases() {
        ...
        // Magisk's grammar separates set members with whitespace, not commas.
        assert!(matches!(
            parse("allow {s1,s2} {t1,t2} c p"),
            Err(ParseError::AvtabAv(_))
        ));
