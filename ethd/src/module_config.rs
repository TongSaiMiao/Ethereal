...
const MODULE_CONFIG_MAGIC: u32 = u32::from_le_bytes(*b"ETHM");
...
/// Validate an identifier before using it as a directory name.
pub fn validate_module_id(module_id: &str) -> Result<()> {
    if module_id.len() > MAX_MODULE_ID_LEN {
        bail!(
            "Module id too long: {} bytes (max: {MAX_MODULE_ID_LEN})",
            module_id.len(),
        );
    }
...
fn get_config_dir(module_id: &str) -> Result<PathBuf> {
    validate_module_id(module_id)?;
    Ok(Path::new(defs::MODULE_CONFIG_DIR).join(module_id))
}

/// Get the config file path for a module
fn get_config_path(module_id: &str, config_type: ConfigType) -> Result<PathBuf> {
    Ok(get_config_dir(module_id)?.join(config_type.filename()))
}
...
mod tests {
    ...
    #[test]
    fn module_id_accepts_normal_names() {
        for id in ["ab", "module.name", "Module_1", "internal-feature"] {
            assert!(validate_module_id(id).is_ok(), "{id}");
        }
    }

    #[test]
    fn module_id_rejects_path_and_shell_syntax() {
        for id in [
            "",
            "a",
            ".hidden",
